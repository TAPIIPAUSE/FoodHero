...
export async function preprocess_House_Score(user) {
  ...
  const member = await get_House_Member(user.hID);
...
export async function preprocess_Org_Score(user) {
  const org_member = await OrganizationScore.find({ orgID: user.orgID })
  ...
  // IMPORTANT!!!
  const currentHouse = await House.findOne({assigned_ID: user.hID})
  const currentHouseName = currentHouse.house_name
  const member = await get_Org_Member(user.orgID);
  ...
  const combined = member.map((m, index) => {
    return {
      ...
      "Housename": m,
      "Score": processed_score_array[index] !== undefined ? processed_score_array[index] : 0,
      "isCurrentUser": currentHouseName == m
    };
  });
...
export async function preprocess_interOrg_Score(user) {
  ...
  const currentOrg = await Organization.findOne({assigned_ID: user.orgID})
  const currentOrgName = currentOrg.org_name
  ...
  const combined = member.map((m, index) => {
    return { 
      "Rank": null,
       "Orgname": m,
        "Score": processed_score_array[index] !== undefined ? processed_score_array[index] : 0,
      "isCurrentUser": currentOrgName == m};
  });
